...
public class CommonMethods extends PageInitializer {
    ...
    public static byte[] takeScreenshot(String imageName) {
        ...
        //this captures the screenshot and store it as a byte array
        byte[] picBytes = ts.getScreenshotAs(OutputType.BYTES);

        //this captures the screenshot and store as file in the sourceFile variable
        File sourcePath = ts.getScreenshotAs(OutputType.FILE);

        try {
            FileUtils.copyFile(sourcePath, new File(Constants.SCREENSHOT_FILEPATH + imageName + getTimeStamp("yyyy-MM-dd-HH-mm-ss")));
        } catch (IOException e) {
    ...
    public static String getTimeStamp(String pattern) // para maging unique pangalan nung image na kokopyahin
    {
        Date date = new Date();
