...
public class APIWorkflowSteps {
    ...
    //mMAKING A REQUEST USING USUAL PAYLOAD
    ...
    //-------------------------------------------------------------------
    //ANOTHER REQUEST USING JSON PAYLOAD - BELOW
    ...
    public void the_status_code_for_creating_an_employee_is(Integer int1) {
        //argument/value is coming from feature file = int1
    ...
    @Given("a request is prepared to get the created employee") //kung may error, malamang dito.
    ...
    @Then("the employee data we get having id {string} must match with global employee_id")
    public void the_employee_data_we_get_having_id_must_match_with_global_employee_id(String string) {
        String tempEmpId = response.jsonPath().getString(string);
        //will store the employee id coming from get call which will be compared to global emp id.
    ...
    public void the_retrieved_data_at_object_matches_with_the_data_of_created_employee
        ...
        List<Map<String, String>> expectedData = dataTable.asMaps(String.class, String.class); //pwede rin wala nitong (String.class)
        Map<String, String> actualData = response.body().jsonPath().get(empObject);

        for (Map<String, String> map : expectedData) {
            Set<String> keys = map.keySet(); //this will store all the keys
            //another enhanced for loop to get values and keys below
            for (String key : keys) {
                String expectedValue = map.get(key);//this will store the value against the key
    ...
    public void a_request_is_prepared_to_create_an_employee_with_dynamic_data
        ...
        request = given().
                ...
                header(APIConstants.HEADER_KEY_AUTHORIZATION, GenerateTokenSteps.token).
                body(APIPayloadConstants.createEmployeePayloadDynamic(firstName, lastName, middleName,
    ...
    @Then("the status code of the updated employee is {int}")
    public void the_status_code_of_the_updated_employee_is(Integer int1) {
        response.then().assertThat().statusCode(int1);
    }
}
